...
========================================================= */
const methodPaths = [];
const previousClassyPrototype = _.cloneDeep(ClassyResource.prototype);
...
async function buildResourceDocs() {
  ...
  Object.keys(resources).forEach(async (resourceKey) => {
    ...
    await new ClassyResource({}, resources[resourceKey]);
  });

  ensureMethodsExist(methodPaths);
}
  ...
  return function(methods) {
    ...
    Object.keys(methods).forEach(function(customMethod) {
      ...
      fs.appendFileSync(file, stripIndent(createCodeBlock(`
        classy.${methodPath}(${args.join(', ')}, options).then(data => {
...
function addRetrieveMethodsOverride(resource, file) {
  ...
  return function(methods) {
    ...
    methods.forEach(function(method) {
      ...
      fs.appendFileSync(file,`\n#### GET /${resourcePathName}/:id/${method}\n`);
      fs.appendFileSync(file, createCodeBlock(`
        classy.${methodPath}(${singular(resourceName)}Id, ${singularLowerCase}Id, options).then(data => {
          // do something with list
        })
      `));
...
/**
 * make sure all the methods generated by the docs are actual
 * methods of the classy node resource. Script fails otherwise.
 */
function ensureMethodsExist(methodPaths) {
  ClassyResource.prototype = previousClassyPrototype;
  console.log(`Validating ${methodPaths.length} methods.`); // eslint-disable-line no-console
  const classy = new Classy({ clientId: 'xxxx', clientSecret: 'yyy' });
  methodPaths.forEach(path => {
    if (typeof _.get(classy, path) !== 'function') {
